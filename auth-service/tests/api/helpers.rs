...
use auth_service::services::data_stores::RedisBannedTokenStore;
use auth_service::services::data_stores::RedisTwoFACodeStore;
...
async fn configure_database(db_conn_string: &SecretString, db_name: &str) {
    ...
    // Run migrations against a new database
    sqlx::migrate!()
        .run(&connection)
        .await
